...
628201729300000000a282828382828252528462b20000a38300a382018283821222324252525252525284525222223200000000000000000000000000000000";
        ...
        // engine
        public Engine engine = new Engine(1500, 1500, 1500, 1500, "Celeste", false);
        ...
        public override void Render()
        {
            Draw.SpriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, null, RasterizerState.CullNone, null, Engine.ScreenMatrix);
            {
                ...
                var pos = new Vector2(buffer.Width*2, buffer.Height*2 ) / 2f;
                ...
                // the buffer / emulator
                Draw.SpriteBatch.Draw(buffer, pos, buffer.Bounds, Color.White, 0f, Vector2.Zero, scale, SpriteEffects.None, 0);
            }
            Draw.SpriteBatch.End();
        ...
        {
            var aimX = MInput.Keyboard.AxisCheck(Keys.Left, Keys.Right);
            var aimY = MInput.Keyboard.AxisCheck(Keys.Up, Keys.Down);
            ...
            if (index == 0)
                ...
                return MInput.Keyboard.Check(Keys.C) || MInput.Keyboard.Check(Keys.Z);
            else if (index == 5)
                return MInput.Keyboard.Check(Keys.X) || MInput.Keyboard.Check(Keys.V);

            return false;
        }

        public Vector2 GetAimVector(int facing)
        {
            Vector2 vec = new Vector2();
            vec.Y = MInput.Keyboard.AxisCheck(Keys.Up, Keys.Down);
            vec.X = MInput.Keyboard.AxisCheck(Keys.Left, Keys.Right);
        ...
        {
            //var axischeck = MInput.Keyboard.AxisCheck(Keys.Left, Keys.Right);
            //return (axischeck != 0) ? axischeck : facing;
            
            return Math.Sign(GetAimVector(facing).X);
        }

        public int dashDirectionY(int facing)
        {
            //return MInput.Keyboard.AxisCheck(Keys.Up, Keys.Down);
            return Math.Sign(GetAimVector(facing).Y);
        }
